module biriscv_multiplier_efficient_n
(
    // Inputs
     input           clk_i
    ,input           rst_i
...
// Local Params
//-----------------------------------------------------------------
localparam [2:0] MULEN_STATE_IDLE  = 3'd0;
localparam [2:0] MULEN_STATE_CALC0 = 3'd1;
localparam [2:0] MULEN_STATE_CALC1 = 3'd2;
localparam [2:0] MULEN_STATE_CALC2 = 3'd3;
localparam [2:0] MULEN_STATE_DONE  = 3'd4;

wire inst_mulen_w = ((opcode_opcode_i & `INST_MULEN_MASK) == `INST_MULEN);
...
begin
    ...
    MULEN_STATE_IDLE:
    begin
        if (opcode_valid_i && inst_mulen_w) // New instruction from issue
        begin
            a_q     <= opcode_ra_operand_i;
            b_q     <= opcode_rb_operand_i;
            rd_idx_q <= opcode_rd_idx_i;  // Latch rd index
            state_q <= MULEN_STATE_CALC0;
        end
    end
    
    MULEN_STATE_CALC0: // P0 = A_l * B_l
    begin
        p0_q    <= mult_out_w; // Latch result computed in THIS cycle
        state_q <= MULEN_STATE_CALC1;
    end
    
    MULEN_STATE_CALC1: // P1 = A_l * B_h
    begin
        p1_q    <= mult_out_w; // Latch result computed in THIS cycle
        state_q <= MULEN_STATE_CALC2;
    end
    
    MULEN_STATE_CALC2: // P2 = A_h * B_l
    begin
        p2_q    <= mult_out_w; // Latch result computed in THIS cycle
        state_q <= MULEN_STATE_DONE;
    end
