...
localparam PASS_PC          = 32'h80000144;
localparam FAIL_PC          = 32'h80000148;
...
initial
begin
    $display("Starting MULP (deep-pipelined multiplier) testbench");
        ...
        if (mem_i_pc_w != last_pc) begin
            ...
            if ((mem_i_pc_w == FAIL_PC) && !pass_reported && !fail_reported) begin
                if (reg_r12_w == expected_result_w) begin
                    pass_reported <= 1'b1;
                    $display("\n*** MULP TEST PASSED! ***");
                    $display("PC reached 0x%08h and MULP matched expectation: x12 = %0d (expected %0d)",
                             FAIL_PC, reg_r12_w, expected_result_w);
                end else begin
                    fail_reported <= 1'b1;
                    $display("\n*** MULP TEST FAILED! ***");
                    $display("PC reached 0x%08h but MULP result incorrect: x12 = %0d (expected %0d)",
                             FAIL_PC, reg_r12_w, expected_result_w);
                end
                $finish;
            end
