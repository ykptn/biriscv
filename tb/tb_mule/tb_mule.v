...
initial begin
    $display("Starting MULE standalone testbench");
...
always @(posedge clk) begin
    if (rst) begin
        ...
        if (u_dut.mule_opcode_valid_w) begin
            if (u_dut.u_issue.pipe1_mux_mule_r)
        ...
        if (u_dut.writeback_mule_valid_w) begin
            $display("[Cycle %0d] MULE WRITEBACK: rd=%0d value=%0d",
                     cycle_count,
        ...
        if (mem_i_pc_w != last_pc) begin
            ...
            if (mem_i_pc_w == PASS_PC && !pass_reported) begin
                if ((reg_r12_w == reg_r13_w) && (reg_r14_w == 32'd5) && pipe1_issue_seen) begin
                    pass_reported <= 1'b1;
                    $display("\n*** MULE TEST PASSED! ***");
                    $display("Cycle %0d reached 0x%08h. MULE (x12) = %0d, MUL (x13) = %0d, pipe1_issue_seen = %0d",
                             cycle_count, PASS_PC, reg_r12_w, reg_r13_w, pipe1_issue_seen);
                    $finish;
                end else begin
                    fail_reported <= 1'b1;
                    $display("\n*** MULE TEST FAILED! ***");
                    $display("PASS loop reached with bad state: MULE x12 = %0d, MUL x13 = %0d, x14 = %0d, pipe1_issue_seen = %0d",
                ...
                fail_reported <= 1'b1;
                $display("\n*** MULE TEST FAILED! ***");
                $display("Reached fail loop: MULE x12 = %0d, MUL x13 = %0d, x14 = %0d, pipe1_issue_seen = %0d",
                         reg_r12_w, reg_r13_w, reg_r14_w, pipe1_issue_seen);
                $finish;
            end
        end
