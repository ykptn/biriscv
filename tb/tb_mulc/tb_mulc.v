...
initial begin
    $display("Starting MULC standalone testbench");
...
always @(posedge clk) begin
    if (rst) begin
        ...
        if (u_dut.cbm_opcode_valid_w) begin
            $display("[Cycle %0d] MULC ISSUE: ra=%0d rb=%0d rd=%0d",
                     cycle_count,
        ...
        if (u_dut.writeback_cbm_valid_w) begin
            $display("[Cycle %0d] MULC WRITEBACK: rd=%0d value=%0d",
                     cycle_count,
        ...
        if (mem_i_pc_w != last_pc) begin
            ...
            if (mem_i_pc_w == PASS_PC && !pass_reported) begin
                if (reg_r12_w == reg_r13_w) begin
                    pass_reported <= 1'b1;
                    $display("\n*** MULC TEST PASSED! ***");
                    $display("Cycle %0d reached 0x%08h. MULC (x12) = %0d, MUL (x13) = %0d",
                             cycle_count, PASS_PC, reg_r12_w, reg_r13_w);
                    $finish;
                end else begin
                    fail_reported <= 1'b1;
                    $display("\n*** MULC TEST FAILED! ***");
                    $display("PASS loop reached but mismatch exists: MULC x12 = %0d, MUL x13 = %0d",
                             reg_r12_w, reg_r13_w);
                    $finish;
                end
                ...
                fail_reported <= 1'b1;
                $display("\n*** MULC TEST FAILED! ***");
                $display("Reached fail loop: MULC x12 = %0d, MUL x13 = %0d, expected (a0*a1 low32) = %0d",
