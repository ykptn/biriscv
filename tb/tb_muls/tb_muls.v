...
initial
begin
    $display("Starting MULS testbench");
        ...
        if (mem_i_pc_w != last_pc) begin
            ...
            if ((mem_i_pc_w == FAIL_PC) && !pass_reported && !fail_reported) begin
                if (reg_r12_w == expected_result_w) begin
                    pass_reported <= 1'b1;
                    $display("\n*** TEST PASSED! ***");
                    $display("PC reached 0x%08h and MULS matched software expectation: x12 = %0d (expected %0d)",
                             FAIL_PC, reg_r12_w, expected_result_w);
                end else begin
                    fail_reported <= 1'b1;
                    $display("\n*** TEST FAILED! ***");
                    $display("PC reached 0x%08h but MULS result incorrect: x12 = %0d (expected %0d)",
                             FAIL_PC, reg_r12_w, expected_result_w);
                end
                $finish;
            end
