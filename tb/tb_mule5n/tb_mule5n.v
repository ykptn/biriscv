...
initial begin
    $display("Starting MULE5N standalone testbench");

    if (`TRACE) begin
        $dumpfile("waveform.vcd");
        $dumpvars(0, tb_mule5n);
    end
...
always @(posedge clk) begin
    if (rst) begin
        ...
        if (u_dut.mule5n_opcode_valid_w) begin
            mule5n_seen <= 1'b1;
            if (u_dut.u_issue.pipe1_mux_mule5n_r)
                pipe1_issue_seen <= 1'b1;
            $display("[Cycle %0d] MULE5N ISSUE: ra=%0d rb=%0d rd=%0d",
                     cycle_count,
                     u_dut.mule5n_opcode_ra_operand_w,
                     u_dut.mule5n_opcode_rb_operand_w,
                     u_dut.mule5n_opcode_rd_idx_w);
        end

        if (mule5n_seen && !mule5n_writeback_seen &&
            ((u_dut.u_issue.opcode_a_issue_r && !u_dut.mule5n_opcode_valid_w) ||
             (u_dut.u_issue.opcode_b_issue_r && !u_dut.mule5n_opcode_valid_w)))
            progress_before_writeback <= 1'b1;

        if (u_dut.writeback_mule5n_valid_w) begin
            mule5n_writeback_seen <= 1'b1;
            $display("[Cycle %0d] MULE5N WRITEBACK: rd=%0d value=%0d",
                     cycle_count,
                     u_dut.writeback_mule5n_rd_idx_w,
                     u_dut.writeback_mule5n_value_w);
        end

        if (!wb_collision_seen &&
            u_dut.writeback_mule5n_valid_w &&
            ...
            $display("[Cycle %0d] MULE5N WB COLLISION: custom rd=%0d value=%0d, pipe0 rd=%0d value=%0d",
                     cycle_count,
                     u_dut.writeback_mule5n_rd_idx_w,
                     u_dut.writeback_mule5n_value_w,
        ...
        if (mem_i_pc_w != last_pc) begin
            ...
            if (mem_i_pc_w == PASS_PC && !pass_reported) begin
                if ((reg_r12_w == reg_r13_w) &&
                    ...
                    pass_reported <= 1'b1;
                    $display("\n*** MULE5N TEST PASSED! ***");
                    $display("Cycle %0d reached 0x%08h. MULE5N x12 = %0d, MUL x13 = %0d, progress_before_writeback = %0d, pipe1_issue_seen = %0d, collision_rd = x%0d",
                             cycle_count, PASS_PC, reg_r12_w, reg_r13_w, progress_before_writeback, pipe1_issue_seen, wb_collision_rd);
                    $finish;
                end else begin
                    fail_reported <= 1'b1;
                    $display("\n*** MULE5N TEST FAILED! ***");
